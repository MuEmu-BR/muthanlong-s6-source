...
BYTE CGuildManager::SetGuildScore(char* szGuildName,DWORD dwScore)
{
	...
	else
	{
		...
		//LogAdd(LOG_GREEN, "UPDATE Guild SET G_Score=%d WHERE G_Name='%s'", dwScore, szGuildName);
		return 1;
	}
}

#if(NEWBOSSGUILD == 1)
BYTE CGuildManager::SetGuildScore1(char* szGuildName, DWORD dwScore1)
{
	GUILD_INFO* lpGuildInfo = this->GetGuildInfo(szGuildName);

	if (lpGuildInfo == 0){ return 0; }

	if (gQueryManager.ExecQuery("UPDATE Guild SET G_Score1=%d WHERE G_Name='%s'", dwScore1, szGuildName) == 0)
	{
		gQueryManager.Close();
		return 0;
	}
	else
	{
		...
		//LogAdd(LOG_BLUE, "UPDATE Guild SET G_Score1=%d WHERE G_Name='%s'", dwScore1, szGuildName);
		return 1;
	}
}
#endif
