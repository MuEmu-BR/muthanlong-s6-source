...
void CAchievements::PlayerReward(int aIndex, int missionIndex)
{
	...
	if (lpObj->ach.counter[missionIndex] >= this->m_AchInfo.at(missionIndex).Count)
	{
		...
		else if (this->m_AchInfo.at(missionIndex).RewardType == 4) // ruud
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, 0, 0, 0, 0, this->m_AchInfo.at(missionIndex).RewardValue);
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[Achievements]: +%d Ruud", this->m_AchInfo.at(missionIndex).RewardValue);
		}
		else if (this->m_AchInfo.at(missionIndex).RewardType == 5) // LevelPoints
		{
			...
			GDCharacterInfoSaveSend(lpObj->Index);//gLog.Output(LOG_SUPER_POINTS,"[SuperPoints]: [%s] [%s] Obtained %d (total %d) points for achievement (id: %d)",lpObj->Account,lpObj->Name, this->m_AchInfo.at(missionIndex).RewardValue,lpObj->iQuestStat,missionIndex);
		}
	}
}
