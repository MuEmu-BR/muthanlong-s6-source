...
bool CSkillManager::SkillManaShield(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	// den doan nay // chỉ là truyền vô đây , vì cái này nhận đầu tiên , 
...
bool CSkillManager::SkillDeathStab(int aIndex, int bIndex, CSkill* lpSkill, bool combo) // OK
{
	...
	if (gServerInfo.SkillDamGioLan == 0)
	{
		if (this->CheckSkillRange(lpSkill->m_index, lpObj->X, lpObj->Y, lpTarget->X, lpTarget->Y) == 0)
		{
			return 0;
	...
	if (gServerInfo.SkillDamGioLan == 1)
	{
...
#if(FIX_VANTHANH == 1)
	if (lpObj->Interface.type == INTERFACE_CHAOS_BOX || lpObj->Interface.type == INTERFACE_CHAOS_BOX)
	{
...
bool CSkillManager::SkillPlasmaStorm(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		gObjAddAttackProcMsgSendDelay(lpObj,53,index,100,lpSkill->m_index,0);
...
void CSkillManager::CGMultiSkillAttackRecv(PMSG_MULTI_SKILL_ATTACK_RECV* lpMsg,int aIndex,bool type) // OK
{
	...
	for(int n=0;n < lpMsg->count;n++)
	{
		...
		//============ Auto Skill Target
		if (!gBProtect.AntiAutoSkill(bIndex))
		{
			return;
		}
		//=============================
...
//#endif
	...
	if
		(
		skill == SKILL_MANA_SHIELD //16 đây là list skill nó dùng để spam
...
#endif
	//============ Auto Skill Target  đây là cách chặn 
	if (!gBProtect.AntiAutoSkill(bIndex))
	{
		return;
	}
