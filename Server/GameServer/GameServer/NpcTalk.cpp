...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	switch(lpNpc->Class)
	{
		...
		case 450:
			...
			#if(SKY_EVENT == 1)
							if (gSkyEvent.NPCEnabled == 1 && lpObj->Map == MAP_DEVIAS){ //<< Chỏ Map em có thể tùy thích cho nó ở map nào thì sửa lại map đó
...
{
#if(CUSTOM_GIOIHAN == 1) // BloodCastle
	if (lpObj->GIOIHAN.DevilSquare >= gCustomGioiHan.m_DevilSquareCount)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gCustomGioiHan.Msg02);
		return;
	}
#endif
