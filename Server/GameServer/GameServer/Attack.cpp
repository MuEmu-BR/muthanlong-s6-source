...
#if(ANTIHACKVT == 1)
	/// chỗ này để fix xả skill tầm xa và chạy nhanh k cho thấy quái
	...
	if(damage == 0)
	{
		...
		if (skill == SKILL_FALLING_SLASH || skill == SKILL_LUNGE || skill == SKILL_UPPERCUT || skill == SKILL_CYCLONE || skill == SKILL_SLASH || skill == SKILL_TWISTING_SLASH || skill == SKILL_RAGEFUL_BLOW || skill == SKILL_DEATH_STAB || skill == SKILL_CRESCENT_MOON_SLASH || skill == SKILL_STAR_FALL || skill == SKILL_IMPALE || skill == SKILL_FIRE_BREATH || skill == SKILL_ICE_ARROW || skill == SKILL_PENETRATION || skill == SKILL_FIRE_SLASH || skill == SKILL_POWER_SLASH || skill == SKILL_SPIRAL_SLASH || skill == SKILL_FROZEN_STAB || skill == SKILL_SWORD_SLASH || skill == SKILL_CHARGE || skill == SKILL_BLOOD_STORM)
		{
			if(skill != SKILL_IMPALE || lpObj->Inventory[8].m_Index == GET_ITEM(13,2) || lpObj->Inventory[8].m_Index == GET_ITEM(13,3) || lpObj->Inventory[8].m_Index == GET_ITEM(13,37))
			{
				...
				{
					damage = (damage*lpObj->DKDamageMultiplierRate) / gServerInfo.DameSkillSocGioDK; // 100
				}
				else
				{
...
void CAttack::DamageSprite(LPOBJ lpObj,int damage) // OK
{
	...
	{
		//lpItem->m_Durability -= (damage*(1.0f*DurabilityValue))/100;
		lpItem->m_Durability -= (damage*(1.0f*DurabilityValue)) / gServerInfo.m_Durability_Pet;
	}
#endif
	else
	{
...
#if(BOSS_GUILD == 1)
	if (lpObj->Map == gBossGuild.m_MapBossGuild)
	{
...
bool CAttack::ApplySkillEffect(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, int damage) // OK
{
	...
	case SKILL_EVIL_SPIRIT:
		...
		//================================================================================================================================================================================================
		//================================================================================================================================================================================================
		if (lpObj->rNguHanh >= 1 && (GetLargeRand() % 100) < gNguHanh.Kim)
		{
			gEffectManager.AddEffect(lpTarget, 0, gSkillManager.GetSkillEffect(SKILL_PLASMA_STORM), 20, lpObj->Index, 2, 3, 0);
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_LIGHTNING_STORM, 10, 0, 0, 0, 0);
		}
		if (lpObj->rNguHanh >= 2 && (GetLargeRand() % 100) < gNguHanh.Moc)
		{
			gEffectManager.AddEffect(lpTarget, 0, gSkillManager.GetSkillEffect(SKILL_POISON), gServerInfo.ThoiGianDameDocNguHanh, lpObj->Index, 2, 3, 0);
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_BLIND, 10, 0, 0, 0, 0);
		}

		if (lpObj->rNguHanh >= 3 && (GetLargeRand() % 100) < gNguHanh.Thuy)
		{
			gEffectManager.AddEffect(lpTarget, 0, gSkillManager.GetSkillEffect(SKILL_ICE_STORM), 20, lpObj->Index, 2, 3, 0);
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_FROZEN_STAB, 10, 0, 0, 0, 0);
		}
		if (lpObj->rNguHanh >= 4 && (GetLargeRand() % 100) < gNguHanh.Hoa)
		{
			gEffectManager.AddEffect(lpTarget, 0, gSkillManager.GetSkillEffect(SKILL_FIRE_SLASH), 20, lpObj->Index, 2, 3, 0);
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_SWORD_SLASH, 20, 0, 0, 0, 0);//ok
		}
		if (lpObj->rNguHanh >= 5 && (GetLargeRand() % 100) < gNguHanh.Tho)
		{
			gEffectManager.AddEffect(lpTarget, 0, gSkillManager.GetSkillEffect(SKILL_SAHAMUTT), 5, lpObj->Index, 2, 3, 0);
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_SAHAMUTT, 10, 0, 0, 0, 0); gEffectManager.AddEffect(lpTarget, 0, EFFECT_NEIL, 10, 0, 0, 0, 0);
		}
		//================================================================================================================================================================================================
		//================================================================================================================================================================================================
	...
	case SKILL_DEATH_STAB:
		gSkillManager.ApplyDeathStabEffect(lpObj, lpTarget, lpSkill, damage);
		//gSkillManager.GCSkillAttackSend(lpObj, SKILL_COMBO, lpTarget->Index, 1);
		//================================================================================================================================================================================================
		//================================================================================================================================================================================================
		if (lpObj->rNguHanh >= 1 && (GetLargeRand() % 100) < gNguHanh.Kim)
		{
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	else
	{
		...
		// Them Moi
		if ((GetLargeRand() % 20) < ((lpObj->rThueVip)))
		{
			(*effect) = 7;
			damage = DamageMax;
			damage += lpObj->rThueVip;

			//LogAdd(LOG_RED, "[TimLoi] rThueVip Monster 0");
		}

		// Them Moi
		if ((GetLargeRand() % 20) < ((lpObj->rDanhHieu)))
		{
			(*effect) = 6;
			damage = DamageMax;
			damage += lpObj->rDanhHieu;

			//LogAdd(LOG_RED, "[TimLoi] rDanhHieu Monster 0");
		}
...
int CAttack::GetAttackDamageWizard(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	if ((GetLargeRand() % 20) < ((lpObj->rThueVip)))
	{
		...
		//LogAdd(LOG_RED, "[TimLoi] rThueVip Monster 1");
	}
	...
	if ((GetLargeRand() % 20) < ((lpObj->rDanhHieu)))
	{
		...
		//LogAdd(LOG_RED, "[TimLoi] rDanhHieu Monster 1");
	}
...
int CAttack::GetAttackDamageCursed(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	if ((GetLargeRand() % 1000) < ((lpObj->CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate))
	{
		(*effect) = 3;
		...
		damage += lpObj->CriticalDamage;
		damage += lpObj->EffectOption.AddCriticalDamage;
		//LogAdd(LOG_RED, "TimLoi] CriticalDamageRate 4");
	}
...
int CAttack::GetAttackDamageFenrir(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	if((GetLargeRand()%100) < ((lpObj->CriticalDamageRate+lpObj->EffectOption.AddCriticalDamageRate)-lpTarget->ResistCriticalDamageRate))
	{
		...
		//LogAdd(LOG_RED, "TimLoi] CriticalDamageRate Soi Tinh");
	}
