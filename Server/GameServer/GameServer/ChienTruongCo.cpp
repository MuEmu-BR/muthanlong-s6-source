...
void cChienTruongCo::LoadData(char * FilePath){
	...
	pugi::xml_node eCTCTimeReset = oChienTruongCo.child("CTCTimeReset");
	for (pugi::xml_node rCTCTimeReset = eCTCTimeReset.child("Config"); rCTCTimeReset; rCTCTimeReset = rCTCTimeReset.next_sibling()){
		this->CTCTimeReset = rCTCTimeReset.attribute("CTCTimeReset").as_int();
	}

	//LogAdd(LOG_RED,"CTC En %d",this->Enabled);
}
...
void cChienTruongCo::SendInfoCTC(int aIndex,int Type) 
{
	...
	pMsg.header.set(0xF3,0x38,sizeof(pMsg)); // a de 38 @@
...
void cChienTruongCo::MonsterDie(LPOBJ Player)
{
	...
	if(Player->Map == 103 && Player->CTCTime > 0) {

		//==== Thuong Point BT
		if(this->PointKillMonter > 0) 
		{
			GDSetCoinSend(Player->Index, +(this->PointKillMonter), 0, 0, "ChienTruongCo");
		...
		if((OBJECT_RANGE(Player->PartyNumber) != 0) && this->PartyKillPoint > 0) 
		{
			...
			for(int n=0;n < MAX_PARTY_USER;n++)
			{
				if(OBJECT_RANGE(lpParty->Index[n]) != 0 && Player->Index != lpParty->Index[n])
				{
					...
					{
						GDSetCoinSend(lpParty->Index[n], +(this->PartyKillPoint), 0, 0, "ChienTruongCo");
...
void cChienTruongCo::CGPacketCTC(XULY_CGPACKET* aRecv, int aIndex)
{
	...
	//if (lpObj->CTCTime < 1)
	//{
	//	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(962));
	//	return;
	//}

	//20-03-2022 sửa lại Chiến Trường Cổ
	...
	else if(ThaoTac == 1 && lpObj->CTCTime > 0) //Vao CTC
	{
		gObjMoveGate(lpObj->Index, 819); //sua gate nay
