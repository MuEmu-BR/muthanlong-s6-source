...
void cSwampEvent::MainProc()
{
	...
	if (this->Enabled == 0)
	{
		if (gServerDisplayer.EventSwampEvent != -1)
		{
			gServerDisplayer.EventSwampEvent = -1;
		}
	}
	else
	{
		if (this->m_iState == SWAMP_STATE_EMPTY)
		{
			gServerDisplayer.EventSwampEvent = this->m_RemainTime;
		}
		else
		{
			if (gServerDisplayer.EventSwampEvent != 0)
			{
				gServerDisplayer.EventSwampEvent = 0;
			}
		}
	}
...
#if(GAMESERVER_TYPE == 0)
	LogAdd(LOG_EVENT, "[Swamp Event] - %s file is Loaded Event ", FilePath);
#endif
...
void cSwampEvent__InsideTrigger(void *  lpParam)
{
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Chuẩn bị cho Vòng tiếp theo!", 1);
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Medusas tấn công: %s", gSwampEvent.pRoom[gSwampEvent.CurrentRoom - 1].Territory);

	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "Tìm Medusa và tiêu diệt chúng!");
	...
	while (Activate)
	{
		...
		if (Seconds >= (gSwampEvent.GroupTimer[gSwampEvent.CurrentGroup - 1] * 60))
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SwampEvent] Bảo vệ không thành công!", 0);
			break;
		}
