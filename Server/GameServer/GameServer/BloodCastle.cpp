...
void CBloodCastle::CGBloodCastleEnterRecv(PMSG_BLOOD_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->GIOIHAN.BloodCastle >= gCustomGioiHan.m_BloodCastleCount)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gCustomGioiHan.Msg01);
		return;
	}
#endif
...
#if(CUSTOM_GIOIHAN == 1) // BloodCastle
	lpObj->GIOIHAN.BloodCastle += 1;
	gCustomGioiHan.GDSaveUserInfo(lpObj->Index);
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gCustomGioiHan.Msg04, lpObj->GIOIHAN.BloodCastle, gCustomGioiHan.m_BloodCastleCount);
#endif

	LogAdd(LOG_RED, "[BloodCastle] Da Vao Trong Blood");
