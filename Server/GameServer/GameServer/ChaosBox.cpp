...
void CChaosBox::Wing2Mix(LPOBJ lpObj, int type) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		if (type == 0)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1601), lpObj->Name, gItemManager.GetItemName(ItemIndex));
...
#if(CUSTOM_MIXDEV == 1)
		lpObj->ChaosWCoinC = lpObj->ChaosSuccessRate * ((CustomMix->m_MixWCoinC < 0) ? 1000 : CustomMix->m_MixWCoinC);
		int TaxWCoinC = (lpObj->ChaosWCoinC*gCastleSiegeSync.GetTaxRateChaos(lpObj->Index)) / 100;
		lpObj->ChaosWCoinC += TaxWCoinC;
#endif
...
#if(CUSTOM_MIXDEV == 1)
		if (lpObj->Coin1 < ((DWORD)lpObj->ChaosWCoinC))
		{
			this->GCChaosMixSend(lpObj->Index, 2, 0);
			LogAdd(LOG_RED, "[ChaosWCoinC] khong du!");
			return;
		}
#endif
...
#if(CUSTOM_MIXDEV == 1)
		//lpObj->Coin1 -= lpObj->ChaosWCoinC;
		GDSetCoinSend(lpObj->Index, -(lpObj->ChaosWCoinC), -(0), -(0), "CustomMix");
...
#if(CUSTOM_MIXDEV == 1)
		gCastleSiegeSync.AddTributeMoney(TaxWCoinC);
#endif
...
void CChaosBox::HornOfFenrirGoldMix(LPOBJ lpObj) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1600), lpObj->Name);
	...
	else
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1603), lpObj->Name);
...
void CChaosBox::VuKhiRongMix(LPOBJ lpObj) // OK
{
	int ChaosPack30 = 0;
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		else if (lpObj->ChaosBox[n].m_Index == GET_ITEM(14, 410))
		{
			FlameCount++;
		}
		else if (lpObj->ChaosBox[n].m_Index == GET_ITEM(14, 414))
		{
			FeatherCount++;
		}
	}

	if (ChaosPack30 != 1 || CreationPack30 != 1 || SoulPack30 != 1 || BlessPack30 != 1 || FlameCount != 1 || FeatherCount != 1)
	{
		this->GCChaosMixSend(lpObj->Index, 6, 0);
		return;
	}

	if (gServerInfo.m_VuKhiRongMixRate[lpObj->AccountLevel] == -1)
	{
		lpObj->ChaosSuccessRate = (ItemMoney / 3000000) + 1;
	}
	else
	{
		lpObj->ChaosSuccessRate = gServerInfo.m_VuKhiRongMixRate[lpObj->AccountLevel];
	}
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		GDCreateItemSend(lpObj->Index, 0xFF, 0, 0, ItemIndex, 0, 0, 1, ItemOption2, ItemOption3, -1, (ItemNewOption + (16 * (GetLargeRand() % 5))), 0, 0, 0, 0, 0xFF, 0);

		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1601), lpObj->Name, gItemManager.GetItemName(ItemIndex));

		gLog.Output(LOG_CHAOS_MIX, "[VuKhiRong][Success][%s][%s] - (ChaosSuccessRate: %d, ChaosMoney: %d)", lpObj->Account, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->ChaosMoney);
	}
	else
	{
		this->ChaosBoxInit(lpObj);
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "%s đã quay thất bại Vũ Khí Rồng", lpObj->Name);

		gLog.Output(LOG_CHAOS_MIX, "[VuKhiRong][Failure][%s][%s] - (ChaosSuccessRate: %d, ChaosMoney: %d)", lpObj->Account, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->ChaosMoney);
	}
}
...
void CChaosBox::CGChaosMixRecv(PMSG_CHAOS_MIX_RECV* lpMsg, int aIndex) // OK
{
	...
	case CHAOS_MIX_FENRIR_GOLD:
		...
		break;
	case CHAOS_MIX_VUKHIRONG: // 140
		this->VuKhiRongMix(lpObj);
		break;
	default:
...
void CChaosBox::GCChaosMixSend(int aIndex, BYTE result, CItem* lpItem) // OK
{
	...
	if (gVanThanhProtect.ThanhTuu == 1)
	{
		if (result == 0)
		{
			gAchievements.FailedChaosMix(aIndex);
		}
	}
#endif
