...
void gObjEventRunProc() // OK
{
	...
	{
		gMuunPetCustom.MainProc(); //21-03-2022 Run Time
	}

#if(MUUNAFK == 1)	//29-03-2022
	if (gMuunAFK.m_Enabled == 1)
	{
		gMuunAFK.MainProc();
		gMuunAFK.MainProcStamina();
	}
#endif
	...
	gCapNhapTime.MainProc();
#endif

#if(GAMESERVER_TYPE==0)
#if(BOSS_NHEN_TIME == 1)
	gBossNhenTime.MainProc();
	gBOSSNHENManager.MainProc();
#endif
#endif
...
short gObjAdd(SOCKET socket,char* IpAddress,int aIndex) // OK
{
	...
	//24-03-2022
	lpObj->AutoSaveMuunTime = GetTickCount();
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				{
					PKEventDataSend(lpObj->Index); // Send về client
				}
#endif
...
#if(VONGQUAY_NEW)
				//27-03-2022
				if (gVanThanhProtect.VongQuay == 1)
				{
		...
		if(lpObj->Connected > OBJECT_LOGGED &&	lpObj->Type == OBJECT_USER)
		{
			...
			//25-03-2022 // update muunpet
			if (GetTickCount() - lpObj->AutoSaveMuunTime > 600000)
			{
				GDCharacterInfoMuunPetSaveSend(lpObj->Index);
				lpObj->AutoSaveMuunTime = GetTickCount();
				LogAdd(LOG_RED, "[GDMuunInventorySaveSend] OK");
			}
