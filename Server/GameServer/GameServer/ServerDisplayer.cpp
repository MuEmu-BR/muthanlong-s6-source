...
void CServerDisplayer::Init(HWND hWnd) // OK
{
	...
	gLog.AddLog(gServerInfo.m_WriteTienTeLog, "LOG_TIEN_TE");

#if(VANTHANH > 3)
	gLog.AddLog(gServerInfo.m_WriteKetNoiLog, "LOG_KET_NOI");

	gLog.AddLog(gServerInfo.m_WriteThuMuaExcLog, "LOG_THU_MUA");
#endif
}
...
void CServerDisplayer::PaintEventTime() // OK
{
	...
	//============================================================================
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "SwampEvent: ");

	if (this->EventSwampEvent == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventSwampEvent == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventSwampEvent;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	else if (this->EventSwampEvent < 300)
	{
		SetTextColor(hdc, RGB(0, 190, 0));
	}
	else
...
#if(LEOTHAP == 1)
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "Event LeoThap: ");

	if (this->EventLeoThap == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventLeoThap == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventLeoThap;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
...
#if(BOSS_NHEN_TIME == 1)
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "Boss Nhen: ");

	for (int n = 0; n < 1; n++)
	{
		SetTextColor(hdc, RGB(0, 102, 204));

		if (this->EventBossNhen[n] == -1)
		{
			wsprintf(text2, "Disabled");
		}
		else if (this->EventBossNhen[n] == 0)
		{
			wsprintf(text2, "Online");
		}
		else
		{
			totalseconds = this->EventBossNhen[n];
			hours = totalseconds / 3600;
			minutes = (totalseconds / 60) % 60;
			seconds = totalseconds % 60;
		...
		TextOut(hdc, posX1, rect.top + 2 + 270, text1, strlen(text1));
		...
		else if (this->EventBossNhen[n] < 300)
		{
			SetTextColor(hdc, RGB(0, 190, 0));
		}
		else
		...
		TextOut(hdc, posX2, rect.top + 2 + 270, text2, strlen(text2));
#endif
	}
	//============================================================================
	SelectObject(hdc, OldFont);
...
#if(THAPBOSS_NEW == 1)
	SetTextColor(hdc, RGB(0, 173, 181));
	TextOut(hdc, rect.left + 5, rect.top + 2, "THAPBOSS:", 9);

	for (int n = 0; n < 10; n++)
	{

		SetTextColor(hdc, RGB(0, 102, 204));
		if (n == 0) { wsprintf(text1, "%d - %s", n, "StartThapBoss:"); }
		if (n == 1) { wsprintf(text1, "%d - %s", n, "ThapBoss-1:"); }
		if (n == 2) { wsprintf(text1, "%d - %s", n, "ThapBoss-2:"); }
		if (n == 3) { wsprintf(text1, "%d - %s", n, "ThapBoss-3:"); }
		if (n == 4) { wsprintf(text1, "%d - %s", n, "ThapBoss-4:"); }
		if (n == 5) { wsprintf(text1, "%d - %s", n, "ThapBoss-5:"); }
		if (n == 6) { wsprintf(text1, "%d - %s", n, "ThapBoss-6:"); }
		if (n == 7) { wsprintf(text1, "%d - %s", n, "ThapBoss-7:"); }
		if (n == 8) { wsprintf(text1, "%d - %s", n, "ThapBoss-8:"); }
		if (n == 9) { wsprintf(text1, "%d - %s", n, "ThapBoss-9:"); }

		if (this->EventThapBoss[n] == -1)
		{
			wsprintf(text2, "Disabled");
		}
		else if (this->EventThapBoss[n] == 0)
		{
			wsprintf(text2, "Online");
		}
		else
		{
			totalseconds = this->EventThapBoss[n];
			hours = totalseconds / 3600;
			minutes = (totalseconds / 60) % 60;
			seconds = totalseconds % 60;
		...
		else if (this->EventThapBoss[n] < 300)
		{
			SetTextColor(hdc, RGB(0, 190, 0));
		}
		else
