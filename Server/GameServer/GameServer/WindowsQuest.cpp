	...
	try
	{
		...
		while (true)
		{
			...
			for (int r = 0; r < EW_MAX_REWARD; r++)
			{
				...
				this->ewdata[a][n].RewardItemLuck[r] = lpMemScript->GetAsNumber();
				this->ewdata[a][n].RewardItemOpt[r] = lpMemScript->GetAsNumber();
				this->ewdata[a][n].RewardItemExl[r] = lpMemScript->GetAsNumber();
...
void ExWinQuestSystem::GC_MainInfo(int aIndex, bool isOpenWindow)
{
	...
	GC_MainPacket pReq;
	pReq.h.set(0xFB, 0x14, sizeof(pReq));
	...
	for (int a = 0; a < EW_MAX_ACT; a++)
	{
		...
		for (int r = 0; r < EW_MAX_REWARD; r++)
		{
			...
			pReq.exQuest[a].RewardItemLuck[r] = this->ewdata[a][n].RewardItemLuck[r];
			pReq.exQuest[a].RewardItemOpt[r] = this->ewdata[a][n].RewardItemOpt[r];
			pReq.exQuest[a].RewardItemExl[r] = this->ewdata[a][n].RewardItemExl[r];
		}
...
void ExWinQuestSystem::CG_AcceptQuest(int aIndex, CG_Accept_Done* aRecv)
{
	...
	if (!lpUser->ExWQuestStart[a])
	{
		...
		{
			//gLog.AddLog(2, "[WinQuestSystem] Error CG_AcceptQuest Result");
			return;
		}
...
void ExWinQuestSystem::RewardQuest(int aIndex, int Act)
{
	...
	{
		//gLog.AddLog(2, "[WinQuestSystem] RewardQuest ERROR");
	...
	for (int r = 0; r < EW_MAX_REWARD; r++)
	{
		...
		rItemLuck = this->ewdata[a][n].RewardItemLuck[r];
		rItemOpt = this->ewdata[a][n].RewardItemOpt[r];
		rItemExcl = this->ewdata[a][n].RewardItemExl[r];
		...
		if (rType == ewBuff)
		{
			gEffectManager.AddEffect(lpObj, 1, rItemType, rCount * 60, 0, 0, 0, 0);
		}
	}
			...
			if (lpMonster->Level >= this->m_Drop[i].MonsterMinLevel && lpMonster->Level <= this->m_Drop[i].MonsterMaxLevel)
			{
				if (lpMonster->Map == this->m_Drop[i].MonsterMap || this->m_Drop[i].MonsterMap == -1)
				{
					if (rand() % 10000 < this->m_Drop[i].ItemDropRate)
					{
						...
						GDCreateItemSend(lpUser->Index, lpMonster->Map, (BYTE)lpMonster->X, (BYTE)lpMonster->Y, GET_ITEM(this->m_Drop[i].ItemType, this->m_Drop[i].ItemIndex), iLevel, this->m_Drop[i].ItemDur, iOption, iLuck, iSkill, gObjMonsterGetTopHitDamageUser(lpMonster), this->m_Drop[i].ItemExcellent, 0, 0, 0, 0, 0xFF, 0);
