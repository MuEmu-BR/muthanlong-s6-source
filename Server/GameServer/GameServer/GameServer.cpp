...
#if(RANKING_NEW == 1)
			//27-03-2022
			gQueueTimer.CreateTimer(QUEUE_TIMER_RANKINGALL, gServerInfo.m_CapNhapXepHangClassAll, &QueueTimerCallback); // 10 Phut
			gQueueTimer.CreateTimer(QUEUE_TIMER_RANKINGCLASS, gServerInfo.m_CapNhapXepHangClass, &QueueTimerCallback); // 30 Phut
			gQueueTimer.CreateTimer(QUEUE_TIMER_RANKINGTOPNAP, gServerInfo.m_CapNhapXepHangTopNap, &QueueTimerCallback); // 20 Phut
			gQueueTimer.CreateTimer(QUEUE_TIMER_RANKINGEVENT, gServerInfo.m_CapNhapXepHangEvent, &QueueTimerCallback); // 25 Phut
#endif
	...
	case WM_COMMAND:
		switch (LOWORD(wParam))
		{
				...
				case IDM_FILE_ALLUSERLOGOUT:
					gObjAllLogOut();
					LogAdd(LOG_RED, "[[KichHoat]Thoat het tai khoan dang nhap)");
					break;
				case IDM_FILE_ALLUSERDISCONNECT:
					gObjAllDisconnect();
					LogAdd(LOG_RED, "[[KichHoat]Ngat ket noi tat ca tai khoan)");
					break;
...
#if(CUSTOM_MIXDEV == 1)
				case IDM_RELOAD_RELOADMIX: //MC bots
					gServerInfo.ReloadCustomMix(); //MC bots
					break;
#endif

				case IDM_RELOAD_RELOADVT: //MC bots
					gServerInfo.ReloadVanThanhInfo(); //MC bots
					break;

				case IDM_RELOAD_RELOADCPANEL: //MC bots
					gServerInfo.ReloadVanThanhInfoCpanel(); //MC bots
					break;

#if(HONCHIENCLASS == 1)
				case IDM_RELOAD_TRANHHUNGPK:
					gServerInfo.ReadPKEventInfo();
					break;
#endif
...
#if(THAPBOSS_NEW == 1)
				case IDM_THAPBOSS0:
					gThapBossManager.StartThapBoss(0);
					break;
				case IDM_THAPBOSS1:
					gThapBossManager.StartThapBoss(1);
					break;
				case IDM_THAPBOSS2:
					gThapBossManager.StartThapBoss(2);
					break;
				case IDM_THAPBOSS3:
					gThapBossManager.StartThapBoss(3);
					break;
				case IDM_THAPBOSS4:
					gThapBossManager.StartThapBoss(4);
					break;
				case IDM_THAPBOSS5:
					gThapBossManager.StartThapBoss(5);
					break;
				case IDM_THAPBOSS6:
					gThapBossManager.StartThapBoss(6);
					break;
				case IDM_THAPBOSS7:
					gThapBossManager.StartThapBoss(7);
					break;
				case IDM_THAPBOSS8:
					gThapBossManager.StartThapBoss(8);
					break;
				case IDM_THAPBOSS9:
					gThapBossManager.StartThapBoss(9);
					break;
#endif

#if(BOSS_NHEN_TIME == 1)
				case IDM_BOSSNHEN:
					gBOSSNHENManager.StartBOSSNHEN(0);
					break;
#endif
