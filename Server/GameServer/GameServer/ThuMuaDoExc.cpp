...
BYTE BotThuMuaer::Alchemy(int aIndex, int BotNum)
{
	...
	{
		LogAdd(LOG_RED, "[BotThuMuaExc]AllowLuck");
	...
	{
		LogAdd(LOG_RED, "[BotThuMuaExc]AllowLevel");
	...
	{
		LogAdd(LOG_RED, "[BotThuMuaExc]AllowSkill");
	...
	{
		LogAdd(LOG_RED, "[BotThuMuaExc]AllowOpt[Check]");
	...
	LogAdd(LOG_RED, "[BotThuMuaExc]AllowOpt[Recv]");
	...
	else
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1602), lpObj->Name, RandomWC, this->bot[BotNum].WCoinP, 0);
	}
...
BOOL BotThuMuaer::TradeOpen(int index, int nindex)
{
	...
	if (this->Enabled == TRUE)
	{
		...
		else
		{
			...
			char wbuf[1024] = { 0 };
			wsprintf(wbuf, "[BotThuMua] (TK(%s)(NV(%s) Open", gObj[index].Account, gObj[index].Name);
			LogAdd(LOG_RED, wbuf);
			ChatSend(&gObj[this->bot[number].index], "Im Ready!");
#if(VANTHANH > 3)
			gLog.Output(LOG_THU_MUA, "[BotThuMua] (TK(%s)(NV(%s) Open", gObj[index].Account, gObj[index].Name);
#endif
