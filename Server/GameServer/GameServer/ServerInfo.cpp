...
void CServerInfo::SendServerInfo(int aIndex)
{
	...
	for (int i = 0; i<MAX_ACCOUNT_LEVEL; i++)
	{
		...
		pMsg.CanOpenMain[i] = gPartySearch.CanOpenMain[i];
		pMsg.CanOpenSettings[i] = gPartySearch.CanOpenSettings[i];
	}
...
#if(CUSTOM_MIXDEV == 1)
	this->ReloadCustomMix();
#endif

#if(VANTHANH > 4)
	this->ReloadVanThanhInfoCpanel();
#endif

	this->ReloadVanThanhInfo();
...
void CServerInfo::ReadCustomInfo() // OK
{
	...
	gCustomJewel.Load(gPath.GetFullPath("Custom\\CustomJewel.xml")); // thay sang xml 
	...
	gShopPointEx.Load(gPath.GetFullPath("ShopPoint.txt"));
	...
	//----------
#if(KINHMACH_NEW == 1)
	if (gVanThanhProtect.KinhMach == 1)
	{
		gKinhMach.Load();
	}
#endif
...
#if(THAPBOSS_NEW == 1)
	gThapBossManager.Load(gPath.GetFullPath("Event\\ThapBoss\\ThapBossManager.dat"));
	gThapBossManager.Init();
#endif

#if(BOSS_NHEN_TIME == 1)
	gBOSSNHENManager.Load(gPath.GetFullPath("Event\\Selupan\\BossNhenManager.dat"));
	gBOSSNHENManager.Init();
#endif
...
void CServerInfo::ReadItemInfo() // OK
{
	...
	LogAdd(LOG_GREEN, "[ServerInfo] Reload Muc Item Thanh Cong");
}
...
void CServerInfo::ReadMoveInfo() // OK
{
	...
	gCustomMove.Load(gPath.GetFullPath("Move\\CustomMove.txt"));
...
#if(VANTHANH > 4)
void CServerInfo::ReloadVanThanhInfoCpanel() // OK
{
	gVanThanhProtect.Load();
	LogAdd(LOG_GREEN, "[ServerInfo] VanThanhCpanel loaded successfully");
}
#endif

#if(CUSTOM_MIXDEV == 1)
void CServerInfo::ReloadCustomMix() // OK
{
	gCustomMix.Load(gPath.GetFullPath("Custom\\CustomMix.txt"));

	LogAdd(LOG_GREEN, "[ServerInfo] CustomMix loaded successfully");
}
#endif

void CServerInfo::ReloadVanThanhInfo() // OK
{
	this->ReloadVanThanhInfo("GameServerInfo", ".\\Data\\VanThanhDev.ini");

#if(MUUNAFK == 1)  //29-03-2022
	gMuunAFK.Read(gPath.GetFullPath("VanThanh\\MuunPetAFK\\MuunPetAFK.txt"));
	gMuunAFK.Load(gPath.GetFullPath("VanThanh\\MuunPetAFK\\MuunPetAFK.xml"));
	gMuunAFK.Init();
	gMuunAFK.InitStamina();

	gMuunAFK.LoadPetWCoinC(gPath.GetFullPath("VanThanh\\MuunPetAFK\\DanhSachMuunPetAFK.txt"));

	gMuunAFK.LoadPetWCoinCStamina(gPath.GetFullPath("VanThanh\\MuunPetAFK\\DanhSachMuunPetStamina.txt"));

#endif

	gMuunPetCustom.Read(gPath.GetFullPath("VanThanh\\MuunPetCustom.txt")); //21-03-2022
	gMuunPetCustom.Init(); //21-03-2022

#if(CAPNHAP_DATASERVER == 1)
	gCapNhapTime.Read(gPath.GetFullPath("VanThanh\\CapNhapDataServer.txt"));
	gCapNhapTime.Init();
#endif

#if(BOSS_NHEN_TIME == 1)
	gBossNhenTime.Read(gPath.GetFullPath("Event\\Selupan\\BossNhenTime.txt"));
	gBossNhenTime.Init();
#endif

#if(CUSTOM_GIOIHAN == 1)
	gCustomGioiHan.Load(gPath.GetFullPath("VanThanh\\CustomGioiHan.xml"));
#endif
...
#if(TAISINH == 1)
	gTaiSinh.Load(gPath.GetFullPath("VanThanh\\CustomTaiSinhTrungSinh.xml"));
#endif
	...
	//----------
#if (GHRS_TOP1_NEW)
	gCustomGHRS.Load(gPath.GetFullPath("VanThanh\\CustomGHRS.ini"));
#endif

	gCustomStartItem.Load(gPath.GetFullPath("VanThanh\\CustomStartItem.txt"));

#if(BUFFSETDAME == 1)
	gCustomStartSetItemDame.Load();
#endif

	gCustomMonster.Load(gPath.GetFullPath("VanThanh\\CustomMonster.txt"));
	gCustomMonsterSkill.Load(gPath.GetFullPath("VanThanh\\CustomMonsterSkill.txt"));

#if(MOCNAP == 1)
	gMocNap.Load(gPath.GetFullPath("VanThanh\\CustomMocNap.txt"));
	gMocNap.Load();
#endif
...
#if(MONSTER_CALL == 1)
	gInvoker.Load(gPath.GetFullPath("VanThanh\\CustomRingMonster.txt"));
#endif

#if (ARCHIVEMENT == 1)
	gAchievements.Read(gPath.GetFullPath("VanThanh\\CustomThanhTuu.xml"));
#endif

#if(CHANGECLASS == 1)
	gChangeClass.Load(gPath.GetFullPath("VanThanh\\CustomDoiGioiTinh.ini"));
#endif
	...
	LogAdd(LOG_GREEN, "[ServerInfo] VanThanh loaded successfully");
}

void CServerInfo::ReloadVanThanhInfo(char* section, char* path) // OK
{
	char GameServerInfo[100] = { 0 };

	//27-03-2022
	this->m_CapNhapXepHangClassAll = GetPrivateProfileInt(section, "CapNhapXepHangClassAll", 0, path);
	this->m_CapNhapXepHangClass = GetPrivateProfileInt(section, "CapNhapXepHangClass", 0, path);
	this->m_CapNhapXepHangTopNap = GetPrivateProfileInt(section, "CapNhapXepHangTopNap", 0, path);
	this->m_CapNhapXepHangEvent = GetPrivateProfileInt(section, "CapNhapXepHangEvent", 0, path);

	//25-03-2022
	this->m_ThongBaoReset = GetPrivateProfileInt(section, "ThongBaoReset", 0, path);

	//23-03-2022
	this->m_ThongBaoOnline = GetPrivateProfileInt(section, "ThongBaoOnline", 0, path);

	this->FenrirofHorse_Drop = GetPrivateProfileInt(section, "FenrirofHorse_Drop", 0, path);
	this->FenrirofHorse_Min = GetPrivateProfileInt(section, "FenrirofHorse_Min", 0, path);
	this->FenrirofHorse_Max = GetPrivateProfileInt(section, "FenrirofHorse_Max", 0, path);

#if(THAPBOSS_NEW == 1)
	this->m_ThapBossSwitch = GetPrivateProfileInt(section, "ThapBossSwitch", 0, path);
#endif

#if(BOSS_NHEN_TIME == 1)
	this->m_BossNhenSwitch = GetPrivateProfileInt(section, "BossNhenSwitch", 0, path);
#endif

#if(MOVE_EVENT_NEW == 1)
	this->GateMoveEvent1 = GetPrivateProfileInt(section, "GateMoveEvent1", 0, path);
	this->GateMoveEvent2 = GetPrivateProfileInt(section, "GateMoveEvent2", 0, path);
	this->GateMoveEvent3 = GetPrivateProfileInt(section, "GateMoveEvent3", 0, path);
	this->GateMoveEvent4 = GetPrivateProfileInt(section, "GateMoveEvent4", 0, path);
	this->GateMoveEvent5 = GetPrivateProfileInt(section, "GateMoveEvent5", 0, path);
	this->GateMoveEvent6 = GetPrivateProfileInt(section, "GateMoveEvent6", 0, path);
	this->GateMoveEvent7 = GetPrivateProfileInt(section, "GateMoveEvent7", 0, path);
	this->GateMoveEvent8 = GetPrivateProfileInt(section, "GateMoveEvent8", 0, path);
	this->GateMoveEvent9 = GetPrivateProfileInt(section, "GateMoveEvent9", 0, path);
	this->GateMoveEvent10 = GetPrivateProfileInt(section, "GateMoveEvent10", 0, path);
	this->GateMoveEvent11 = GetPrivateProfileInt(section, "GateMoveEvent11", 0, path);
	this->GateMoveEvent12 = GetPrivateProfileInt(section, "GateMoveEvent12", 0, path);
	this->GateMoveEvent13 = GetPrivateProfileInt(section, "GateMoveEvent13", 0, path);
	this->GateMoveEvent14 = GetPrivateProfileInt(section, "GateMoveEvent14", 0, path);
	this->GateMoveEvent15 = GetPrivateProfileInt(section, "GateMoveEvent15", 0, path);
	this->GateMoveEvent16 = GetPrivateProfileInt(section, "GateMoveEvent16", 0, path);
	this->GateMoveEvent17 = GetPrivateProfileInt(section, "GateMoveEvent17", 0, path);
	this->GateMoveEvent18 = GetPrivateProfileInt(section, "GateMoveEvent18", 0, path);
	this->GateMoveEvent19 = GetPrivateProfileInt(section, "GateMoveEvent19", 0, path);
	this->GateMoveEvent20 = GetPrivateProfileInt(section, "GateMoveEvent20", 0, path);
	this->GateMoveEvent21 = GetPrivateProfileInt(section, "GateMoveEvent21", 0, path);
	this->GateMoveEvent22 = GetPrivateProfileInt(section, "GateMoveEvent22", 0, path);
	this->GateMoveEvent23 = GetPrivateProfileInt(section, "GateMoveEvent23", 0, path);
	this->GateMoveEvent24 = GetPrivateProfileInt(section, "GateMoveEvent24", 0, path);
	this->GateMoveEvent25 = GetPrivateProfileInt(section, "GateMoveEvent25", 0, path);
	this->GateMoveEvent26 = GetPrivateProfileInt(section, "GateMoveEvent26", 0, path);
	this->GateMoveEvent27 = GetPrivateProfileInt(section, "GateMoveEvent27", 0, path);
	this->GateMoveEvent28 = GetPrivateProfileInt(section, "GateMoveEvent28", 0, path);
	this->GateMoveEvent29 = GetPrivateProfileInt(section, "GateMoveEvent29", 0, path);
	this->GateMoveEvent30 = GetPrivateProfileInt(section, "GateMoveEvent30", 0, path);
	this->GateMoveEvent31 = GetPrivateProfileInt(section, "GateMoveEvent31", 0, path);
	this->GateMoveEvent32 = GetPrivateProfileInt(section, "GateMoveEvent32", 0, path);
	this->GateMoveEvent33 = GetPrivateProfileInt(section, "GateMoveEvent33", 0, path);
	this->GateMoveEvent34 = GetPrivateProfileInt(section, "GateMoveEvent34", 0, path);
	this->GateMoveEvent35 = GetPrivateProfileInt(section, "GateMoveEvent35", 0, path);
	this->GateMoveEvent36 = GetPrivateProfileInt(section, "GateMoveEvent36", 0, path);
	this->GateMoveEvent37 = GetPrivateProfileInt(section, "GateMoveEvent37", 0, path);
	this->GateMoveEvent38 = GetPrivateProfileInt(section, "GateMoveEvent38", 0, path);
	this->GateMoveEvent39 = GetPrivateProfileInt(section, "GateMoveEvent39", 0, path);
	this->GateMoveEvent40 = GetPrivateProfileInt(section, "GateMoveEvent40", 0, path);
	this->GateMoveEvent41 = GetPrivateProfileInt(section, "GateMoveEvent41", 0, path);
	this->GateMoveEvent42 = GetPrivateProfileInt(section, "GateMoveEvent42", 0, path);
	this->GateMoveEvent43 = GetPrivateProfileInt(section, "GateMoveEvent43", 0, path);
	this->GateMoveEvent44 = GetPrivateProfileInt(section, "GateMoveEvent44", 0, path);
	this->GateMoveEvent45 = GetPrivateProfileInt(section, "GateMoveEvent45", 0, path);
	this->GateMoveEvent46 = GetPrivateProfileInt(section, "GateMoveEvent46", 0, path);
	this->GateMoveEvent47 = GetPrivateProfileInt(section, "GateMoveEvent47", 0, path);
	this->GateMoveEvent48 = GetPrivateProfileInt(section, "GateMoveEvent48", 0, path);
	this->GateMoveEvent49 = GetPrivateProfileInt(section, "GateMoveEvent49", 0, path);
	this->GateMoveEvent50 = GetPrivateProfileInt(section, "GateMoveEvent50", 0, path);
#endif

	this->CapDoItemDongVang = GetPrivateProfileInt(section, "CapDoItemDongVang", 13, path);

	this->CustomAttackDiChuyen = GetPrivateProfileInt(section, "CustomAttackDiChuyen", 0, path);

	this->m_GuildPK = GetPrivateProfileInt(section, "GuildDisablePK", 0, path);
	...
	this->ColorDame = GetPrivateProfileInt(section, "ColorDame", 0, path);

	this->m_Durability_Pet = GetPrivateProfileInt(section, "m_Durability_Pet", 0, path);

	this->MoRongHomDo = GetPrivateProfileInt(section, "MoRongHomDo", 0, path);
	...
	// --- ngũ hành
	this->ThoiGianDameDocNguHanh = GetPrivateProfileInt(section, "ThoiGianDameDocNguHanh", 0, path);
	...
	// Mix
	this->m_VuKhiRongMixRate[0] = GetPrivateProfileInt(section, "VuKhiRongMixRate_AL0", 0, path);
	this->m_VuKhiRongMixRate[1] = GetPrivateProfileInt(section, "VuKhiRongMixRate_AL1", 0, path);
	this->m_VuKhiRongMixRate[2] = GetPrivateProfileInt(section, "VuKhiRongMixRate_AL2", 0, path);
	this->m_VuKhiRongMixRate[3] = GetPrivateProfileInt(section, "VuKhiRongMixRate_AL3", 0, path);


#if(LEOTHAP == 1)
	// Event
	this->m_LeoThapEvent = GetPrivateProfileInt(section, "LeoThapEvent", 0, path);
	this->m_LeoThapMaxUser = GetPrivateProfileInt(section, "LeoThapMaxUser", 0, path);
...
void CServerInfo::ReadPKEventInfo() // OK
{
	...
	GetPrivateProfileString("MESSAGES", "Msg01", "[Survivor][%s] Event starts in %d minutes", this->SurvivorMsg[0], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg02", "[Survivor][%s] Enter /survivor to join the Event", this->SurvivorMsg[1], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg03", "All Classes", this->SurvivorMsg[2], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg04", "Wizard", this->SurvivorMsg[3], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg05", "Knight", SurvivorMsg[4], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg06", "Elf", SurvivorMsg[5], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg07", "Gladiator", this->SurvivorMsg[6], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg08", "Dark Lord", this->SurvivorMsg[7], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg09", "Rage Fighter", this->SurvivorMsg[8], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg10", "The current Survivor event is not for your class", this->SurvivorMsg[9], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg11", "No Survivor event is currently opened", this->SurvivorMsg[10], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg12", "You are signed to Survivor! Good luck!", this->SurvivorMsg[11], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg13", "You are already signed for next Survivor!", this->SurvivorMsg[12], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg14", "[Survivor] Battle starts in %d minutes. Prepare yourself!", this->SurvivorMsg[13], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg15", "[Survivor] Battle has started. Save yourself!", this->SurvivorMsg[14], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg16", "[%s] killed [%s]", this->SurvivorMsg[15], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg17", "You were killed by %s", this->SurvivorMsg[16], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg18", "[Survivor] [%s] is the Survivor, and Wins!", this->SurvivorMsg[17], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg19", "[Survivor] [%s] Wins by kill count", this->SurvivorMsg[18], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg20", "[Survivor] Event ended without winner at this time", this->SurvivorMsg[19], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg21", "[Survivor] Event canceled due to lack of players", this->SurvivorMsg[20], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg22", "You cant clear PK during the Survivor event", this->SurvivorMsg[21], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	GetPrivateProfileString("MESSAGES", "Msg23", "The current Survivor event is not for your class", this->SurvivorMsg[22], sizeof(this->SurvivorMsg[0]), "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorMap = GetPrivateProfileInt("OPTIONS", "SurvivorMap", 0, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorX1 = GetPrivateProfileInt("OPTIONS", "SurvivorX1", 136, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorY1 = GetPrivateProfileInt("OPTIONS", "SurvivorY1", 123, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorX2 = GetPrivateProfileInt("OPTIONS", "SurvivorX2", 145, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorY2 = GetPrivateProfileInt("OPTIONS", "SurvivorY2", 131, "..\\Data\\Custom\\PKEvent.ini");

	this->SurvivorTimeLimit = GetPrivateProfileInt("OPTIONS", "TimeLimit", 5, "..\\Data\\Custom\\PKEvent.ini");
    this->SurvivorTimeData = GetPrivateProfileInt("OPTIONS", "TimeDataSQL", 5, "..\\Data\\Custom\\PKEvent.ini");
	
	this->SurvivorTimeThongBaoDangKy = GetPrivateProfileInt("OPTIONS", "TimeThongBaoEvent", 5, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorTimeChuanBiBatDau = GetPrivateProfileInt("OPTIONS", "TimeChuanBiChienDau", 5, "..\\Data\\Custom\\PKEvent.ini");


	this->SurvivorWinnerByKillCount = GetPrivateProfileInt("OPTIONS", "WinnerByKillCount", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorMinimumPlayers = GetPrivateProfileInt("OPTIONS", "MinimumPlayers", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorDisableTeleport = GetPrivateProfileInt("OPTIONS", "DisableTeleport", 1, "..\\Data\\Custom\\PKEvent.ini");

	this->SurvivorItemByKillCharter = GetPrivateProfileInt("DropItem", "DropItem", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorItemType = GetPrivateProfileInt("DropItem", "ItemType", 1, "..\\Data\\Custom\\PKEvent.ini");

	this->SurvivorItemIndex1 = GetPrivateProfileInt("DropItem", "ItemIndex1", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorItemIndex2 = GetPrivateProfileInt("DropItem", "ItemIndex2", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorItemIndex3 = GetPrivateProfileInt("DropItem", "ItemIndex3", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorItemIndex4 = GetPrivateProfileInt("DropItem", "ItemIndex4", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorItemIndex5 = GetPrivateProfileInt("DropItem", "ItemIndex5", 1, "..\\Data\\Custom\\PKEvent.ini");

	this->SurvivorWinWCoinC = GetPrivateProfileInt("PhanThuong", "WCoinC", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorWinWCoinP = GetPrivateProfileInt("PhanThuong", "WCoinP", 1, "..\\Data\\Custom\\PKEvent.ini");
	this->SurvivorWinThanhTuu = GetPrivateProfileInt("PhanThuong", "ThanhTuu", 1, "..\\Data\\Custom\\PKEvent.ini");
