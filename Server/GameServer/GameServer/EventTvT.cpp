		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_EventTimeReg = lpMemScript->GetAsNumber();
...
bool CTvTEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	// SETtIME
	lpObj->PKEventTime = this->m_EventTimeReg;
...
void CTvTEvent::TimeTvTEvent(int aIndex)
{
	...
	if (lpObj->PKEventTime < 0) { lpObj->PKEventTime = 0; }
	...
	if (this->GetState() == TVT_EVENT_STATE_START)
	{
		if (lpObj->PKEventTime >= 1)
		{
			...
			this->SendInfoTimeTvTEvent(lpObj->Index); //Cap Nhat
		}
	}
}

void CTvTEvent::SendInfoTimeTvTEvent(int aIndex)
{
	...
	INFOTVTEVENT_SENDCLIENT pMsg;
	pMsg.header.set(0xF3, 103, sizeof(pMsg)); // a de 38 @@
	pMsg.TimeConLaiTvTEvent = lpObj->PKEventTime;
