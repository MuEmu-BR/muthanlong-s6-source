...
void CChaosCastle::SetState_START(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	if (gVanThanhProtect.ThanhTuu == 1)
	{
		for (int n = 0; n < MAX_CC_USER; n++)
		{
			...
			{
				gAchievements.TeleportInCC(lpLevel->User[n].Index);
			}
		}
	}
#endif
	...
	for(int n=0;n < MAX_CC_USER;n++)
	{
		if(OBJECT_RANGE(lpLevel->User[n].Index) != 0)
		{
			if(lpLevel->User[n].Index == lpLevel->WinnerIndex)
			{
				...
				if (gVanThanhProtect.ThanhTuu == 1)
				{
					gAchievements.WinCC(lpLevel->User[n].Index);
				}
#endif
...
#if(CUSTOM_GIOIHAN == 1) // Devil
	if (lpObj->GIOIHAN.ChaosCastle >= gCustomGioiHan.m_ChaosCastleCount)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gCustomGioiHan.Msg03);
		return;
	}
#endif
...
#if(CUSTOM_GIOIHAN == 1) // BloodCastle
	lpObj->GIOIHAN.ChaosCastle += 1;
	gCustomGioiHan.GDSaveUserInfo(lpObj->Index);
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gCustomGioiHan.Msg06, lpObj->GIOIHAN.ChaosCastle, gCustomGioiHan.m_ChaosCastleCount);
#endif
	LogAdd(LOG_RED, "[ChaosCastle] Da Vao Trong ChaosCastle");
}
