	...
	{ 
		if (this->WinnerNumber >= 0)
		{
			...
			if(this->m_RemainTimeWinner == 15) //Trao thuong cho Guild
			{
				...
				{
					LogAdd(LOG_EVENT,"[Công Thành chiến Mini] Guild Winner: %s", lpGuildInfo->Name);
					...
					for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
					{
						if(gObjIsConnectedGP(n) != 0 && gObj[n].Type == OBJECT_USER)
						{
							...
							{
								LogAdd(LOG_EVENT,"[Công Thành chiến Mini] Bonus Guild Member Reward: %s", gObj[n].Name);
			...
			if(this->m_RemainTimeWinner <= 0)
			{
				...
				LogAdd(LOG_EVENT,"[Công Thành chiến Mini] Finish 1");
				for(int n=0;n < MAX_CTC_CHAR;n++)
				{
		...
		{
			LogAdd(LOG_EVENT,"[Công Thành chiến Mini] Finish 2");
...
void CCTCmini::SetState_START() // OK
{
	...
	LogAdd(LOG_EVENT,"[Công Thành chiến Mini] Bat Dau Event Cong Thanh");
	...
	if((lpMonster->Class == ClassTru || lpMonster->Class == ClassCong) && lpMonster->Map == MapCTC)
	{
		for(int n=0;n < 3;n++) //Check Kill Tru
		{
			...
			if(IndexTru[n] == lpMonster->Index)
			{
				...
				LogAdd(LOG_EVENT,"[Công Thành chiến Mini] Bonus Pha Tru %s", lpUser->Name);
...
void CCTCmini::StartCTCMini()
{
	...
	LogAdd(LOG_EVENT,"[Set CTC Mini Start] At %2d:%2d:00",hour,minute);
