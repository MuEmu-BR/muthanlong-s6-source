...
void cSkyEvent::MainProc()
{
	...
	if (this->Enabled == 0)
	{
		if (gServerDisplayer.EventSkyEvent != -1)
		{
			gServerDisplayer.EventSkyEvent = -1;
		}
	}
	else
	{
		if (this->m_State == SKY_STATE_EMPTY)
		{
			gServerDisplayer.EventSkyEvent = this->m_RemainTime;
		}
		else
		{
			if (gServerDisplayer.EventSkyEvent != 0)
			{
				gServerDisplayer.EventSkyEvent = 0;
			}
		}
	}
...
void cSkyEvent::ProcState_EMPTY()
{
	if (this->Start == 0)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SkyEvent] Event Đã bắt đầu !");
...
void cSkyEvent__InsideTrigger(void * lpParam)	//OK
{
	...
	for (int i = 5; i > 0; i--) //5
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Sky Event] sẽ đóng sau %d phút !", i);
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SkyEvent] Lối vào sự kiện đã bị đóng");
	...
	for (int iStages = 0; iStages < MAX_SKY_EVENT_STAGES; iStages++)
	{
		...
		gSkyEvent.MsgStringSend("[SkyEvent] Trải nghiệm sân khấu đã thay đổi!", 0x01);

		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[SkyEvent] Giai đoạn %d Thời lượng: %d Phút", iStages + 1, gSkyEvent.StageMinutes[iStages]);
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Sky Event] Event Kết Thúc !");

	for (int i = 2; i > 0; i--)
	{
		wsprintf(sBuf, "%d Giây trở về Devias", i * 10);
			...
			if (gObj[m_Index].Connected >= OBJECT_CONNECTED)
			{
				if (this->m_UserData[iLevel][iUser].DBNumber == gObj[m_Index].DBNumber)
				{
					if ((gObj[this->m_UserData[iLevel][iUser].m_Index].Map == MAP_ICARUS) && (gObj[this->m_UserData[iLevel][iUser].m_Index].X >= 130))
					{
						...
						{
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index, 0, 0, 0, 0, 0, "[SkyEvent] - Kinh Nghiệm: %d", this->ExtraExp[iLevel][0]);
						}
						if (this->CurrentStage == 2)
						{
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index, 0, 0, 0, 0, 0, "[SkyEvent] - Kinh Nghiệm: %d", this->ExtraExp[iLevel][1]);
						}
						if (this->CurrentStage == 3)
						{
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index, 0, 0, 0, 0, 0, "[SkyEvent] - Kinh Nghiệm: %d", this->ExtraExp[iLevel][2]);
						}
	...
	if ((this->Enabled == 1) && (this->Start == 1) && (this->NPCEnabled == 1))
	{
		...
		if (iLevel > -1)
		{
			if (this->PlayerCount[iLevel] < MAX_SKY_EVENT_SUB_LEVEL)
			{
				...
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn cần %s + %d để tham gia Cấp độ sự kiện Sky Event %d!", ItemAttribute[0x1C33].Name, iLevel + 1, iLevel + 1);
				}
			}
			else
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "[Sky Event][%d] đã đầy người!", iLevel + 1);
...
void cSkyEvent::AddUser(BYTE iLevel, int iIndex, LPOBJ lpObj)	//OK
{
	...
	gNotice.NewNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, "[SkyEvent] Tổng số người %d", this->PlayerCount[iLevel]);
