...
#if(CUSTOM_MIXDEV == 1)
					info.m_MixWCoinC = lpMemScript->GetAsNumber();
					// load viet chữ
					strcpy_s(info.m_MixSuccess, lpMemScript->GetAsString());
					// load viet chữ
					strcpy_s(info.m_MixFailure, lpMemScript->GetAsString());
#endif

					this->m_Data_Mix.insert(std::pair<int,CUSTOM_MIX>(info.m_Index, info));

					//LogAdd(LOG_RED, "[CUSTOM_MIX] %d", info.m_OpcionExeRand);
