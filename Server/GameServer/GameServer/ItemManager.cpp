...
#if(FIXMUUNPET == 1)
	// khong the deo len o
	if (lpItem->m_Index >= GET_ITEM(13, 200) && lpItem->m_Index <= GET_ITEM(13, 245))
	{
		return 0;
	}
#endif
...
#if(MONSTER_CALL == 1)
		if (gInvoker.InvokerSummon(aIndex) != 0)
		{
			gObjViewportListProtocolCreate(&gObj[aIndex]);
			gObjectManager.CharacterUpdateMapEffect(&gObj[aIndex]);
		}
...
int CItemManager::RepairItem(LPOBJ lpObj, CItem* lpItem, int slot, int type) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "{FIX13} - Acceso prohibido.");
		return 0;
	}
	//--
...
#if(FIXMUUNPET == 1)
	if (lpObj->Inventory[8].m_Index >= GET_ITEM(13, 200) && lpObj->Inventory[8].m_Index <= GET_ITEM(13, 245))
	{
		return 0;
	}
#endif
...
#if(VANTHANH > 4)
	if (gServerInfo.FenrirofHorse_Drop == 1)
	{
		if (lpItem->m_Index >= GET_ITEM(13, gServerInfo.FenrirofHorse_Min) && lpItem->m_Index <= GET_ITEM(13, gServerInfo.FenrirofHorse_Max))
		{
			LogAdd(LOG_RED, "[drop item fenrrir of horse] khong the dap ra");
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
	}
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		//LogAdd(LOG_RED, "CGItemSellRecv 3 Muun -> %d", lpObj->MuunInventory[lpMsg->slot].IsItem());
		return;
	}
	...
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		//LogAdd(LOG_RED, "CGItemSellRecv 4");
		return;
	}
...
#if(FIXMUUNPET == 1)
				if ((lpObj->Inventory[aRecv->Source].m_Index >= GET_ITEM(13, 200) && lpObj->Inventory[aRecv->Source].m_Index <= GET_ITEM(13, 245)))
				{
					//LogAdd(LOG_RED, "khong the deo pet len");
					return;
				}
#endif
