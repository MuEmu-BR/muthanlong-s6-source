	...
	switch(head)
	{
		...
		case 0xD9:
			...
			case 0x02:
				...
				break;
			case 0x03:
				gRanking.RecvDSDW((DGCharTopDW*)lpMsg);
				break;
			case 0x04:
				gRanking.RecvDSDK((DGCharTopDK*)lpMsg);
				break;
			case 0x05:
				gRanking.RecvDSEF((DGCharTopEF*)lpMsg);
				break;
			case 0x06:
				gRanking.RecvDSMG((DGCharTopMG*)lpMsg);
				break;
			case 0x07:
				gRanking.RecvDSDL((DGCharTopDL*)lpMsg);
				break;
			case 0x08:
				gRanking.RecvDSSM((DGCharTopSM*)lpMsg);
				break;
			case 0x09:
				gRanking.RecvDSRF((DGCharTopRF*)lpMsg);
				break;
			case 0x10:
				gRanking.RecvDSNAP((DGCharTopNAP*)lpMsg);
				break;
			case 0x11:
				gRanking.RecvDSBLOOD((DGCharTopBLOOD*)lpMsg);
				break;
			case 0x12:
				gRanking.RecvDSDEVIL((DGCharTopDEVIL*)lpMsg);
				break;
			case 0x13:
				gRanking.RecvDSCHAOS((DGCharTopCHAOS*)lpMsg);
				break;
			}
			break;
#endif
...
#if(CUSTOM_GIOIHAN == 1)
			case 0x05: //-> Answer Hornor Data
				gCustomGioiHan.DGGetData((GIOIHAN_DG_GET_DATA*)lpMsg);
				break;
#endif

#if(MUUNAFK == 1) //29-03-2022
			case 0x06:
				gMuunAFK.DGGetData((MUUNAFK_DG_GET_DATA*)lpMsg);
				break;
#endif

#if(PHUKIEN_NEW == 1)
			case 0x07: //-> Answer Hornor Data
				gPhuKien.DGGetData((PHUKIEN_DG_GET_DATA*)lpMsg);
				break;
#endif

#if(KINHMACH_NEW == 1)
			case 0x08: //-> 
				gKinhMach.DGGetData((KINHMACH_DG_GET_DATA*)lpMsg);
				break;
#endif
...
#if(CHIENTRUONGCO == 1)
            case 0xFC: //Info Tao Tuong Nhan vat
...
#if(BOT_STATUE == 1)
		case 0xF9: //Info Tao Tuong Nhan vat
		//{
		...
		case 0xFE:
		{
					 if (lpMsg[0] == 0xC2 && lpMsg[4] == 0x00)
					 {
						 g_ChoTroi.DGAnsItemList((SDHP_ANS_MARKET_ITEM*)lpMsg);
					 }

					 if (lpMsg[0] == 0xC1 && lpMsg[3] == 0x01)
					 {
						 g_ChoTroi.DGAnsItemSell((SDHP_ANS_MARKET_SELL*)lpMsg);
					 }

					 if (lpMsg[0] == 0xC1 && lpMsg[3] == 0x02)
					 {
						 g_ChoTroi.DGAnsItemBuy((SDHP_ANS_MARKET_BUY*)lpMsg);
					 }

					 if (lpMsg[0] == 0xC1 && lpMsg[3] == 0x03)
					 {
						 g_ChoTroi.DGAnsItemStatus((SDHP_ANS_MARKET_STATUS*)lpMsg);
					 }
		}
			break;
#endif
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	if((lpObj->Map == 98)) 
	{ //Move về lorencia khi thoát game vào lại, nếu thay đổi thì cần đồng map 98
	...
	//{
		gKinhMach.GDReqData(lpObj->Index);
...
#if (CUSTOM_GIOIHAN == 1)
	gCustomGioiHan.GDReqData(lpObj->Index);
#endif

#if (MUUNAFK == 1) // 29-03-2022
	gMuunAFK.GDReqData(lpObj->Index);
#endif

#if(PHUKIEN_NEW == 1)
	gPhuKien.GDReqData(lpObj->Index);
#endif
#if(NGUHANH_NEW == 1)
	//27-03-2022
	...
	gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, "Giới hạn Resets hôm này là %d", gCustomGHRS.maxResets);

#if(MONSTER_CALL == 1)
	//27-03-2022
	if (gVanThanhProtect.MonsterCall == 1)
	{
	...
	else
	{
		...
		{
#if GAMESERVER_CLIENTE_UPDATE >= 2 
			if (gServerInfo.m_ThongBaoOnline == 1)
			{
				PostMessageUserON(lpObj->Name, gMessage.GetMessage(640));
			}
#endif
			if (gServerInfo.m_ThongBaoOnline == 2)
			{
				//23-03-2022 Thông báo all Sub
				char szTemp1[256];
				wsprintf(szTemp1, gMessage.GetMessage(640), lpObj->Name);
				//----
				short tmpservercode = gServerInfo.m_ServerCode;
				while (tmpservercode >= 20) { tmpservercode -= 20; }
				tmpservercode += 1;
				char szTemp[256];
				wsprintf(szTemp, "[Sub-%d]", tmpservercode);
				GDGlobalPostSend(gMapServerManager.GetMapServerGroup(), 2, szTemp, szTemp1);
				//---
			}
		}
	}
	...
	//27-03-2022
	if (gVanThanhProtect.DoiClass == 1)
	{
...
#if(DEV_PLAYERSHOP == 1)
	//27-03-2022
	if (gVanThanhProtect.ChoTroi == 1)
	{
		g_ChoTroi.GDReqItemStatus(lpObj->Index);
	}
#endif

#if(NGANHANGNGOC_NEW == 1)
	//27-03-2022
	if (gVanThanhProtect.NganHangNgoc == 1)
	{
...
#if(DANHHIEU_NEW == 1)
	//27-03-2022
	if (gVanThanhProtect.DanhHieu == 1)
	{
...
#if(PHUKIEN_NEW == 1)
	//27-03-2022
	if (gVanThanhProtect.PhuKien == 1)
	{
		PhuKienDataSend(lpObj->Index);
	}
#endif

	GifCodeDataSend(lpObj->Index);

#if(VANTHANH > 4)
	VanThanhDataSend(lpObj->Index);
#endif

	LogAddConnect(LOG_BLUE,"[Obj][%d] LogInCharacter (%s)",lpObj->Index,lpObj->Name);
#if(VANTHANH > 3)
	gLog.Output(LOG_KET_NOI, "[Obj][%d] LogInCharacter (%s)", lpObj->Index, lpObj->Name);
#endif
}
...
void GDSaveCapNhapTimeData(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	CAPNHAPTIME_GD_SAVE_DATA pRequest;
...
#if(LEOTHAP == 1)
	pMsg.TowerCount = lpObj->TowerCount;
	pMsg.TowerTime = lpObj->TowerTime;
#endif
...
#if(NGANHANGNGOC_NEW == 1)
	pMsg.JMoney = lpObj->JMoney;
	pMsg.JBless = lpObj->JBless;
	pMsg.JSoul = lpObj->JSoul;
	pMsg.JChaos = lpObj->JChaos;
	pMsg.JCreation = lpObj->JCreation;
	pMsg.JLife = lpObj->JLife;
	pMsg.JHarmony = lpObj->JHarmony;
	pMsg.JGuaStone = lpObj->JGuaStone;
	pMsg.JGemStone = lpObj->JGemStone;
	pMsg.JBigStone = lpObj->JBigStone;
	pMsg.JSmallStone = lpObj->JSmallStone;
#endif
...
//25-03-2022
void GDCharacterInfoMuunPetSaveSend(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, char * LogName) // OK
{
    ...
    gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.header.size);
		...
		{
			lpObj->Coin1 == 0;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(760), 0);
		}
				...
				//2-04-2022
				gCashShop.GCSendCoin(lpObj, lpObj->Coin1, lpObj->Coin2, lpObj->Coin3);

		LogAdd(LOG_ALERT,"[%s][%s] WZ_SetCoin(%s) Val1: %d, Val2: %d, Val3: %d",lpObj->Account,lpObj->Name,LogName,value1,value2,value3);

		gLog.Output(LOG_TIEN_TE, "[%s][%s] WZ_SetCoin(%s) Val1: %d, Val2: %d, Val3: %d", lpObj->Account, lpObj->Name, LogName, value1, value2, value3);

		gLog.Output(LOG_TIEN_TE, "[%s][%s] WCoinC Hien Tai: %d", lpObj->Account, lpObj->Name, lpObj->Coin1);
