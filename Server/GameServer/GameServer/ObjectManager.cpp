...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	//25-03-2022
	GDCharacterInfoMuunPetSaveSend(aIndex);
...
#if(BOSS_GUILD == 1)
	else if (lpObj->Map == gBossGuild.m_MapBossGuild)
	{
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	while(change != 0)
	{
		...
		{
			gKinhMach.Option(lpObj, 1);
		}
#endif
		//---------
#if (SACHTHUOCTINH_NEW == 1)
		gSachThuocTinh.Option(lpObj,1);
#endif
		//---------
#if (PHUKIEN_NEW == 1)
		gPhuKien.Option(lpObj, 1);
#endif
		//---------
#if(BUFFSETDAME == 1)
	...
	if (gVanThanhProtect.KinhMach == 1)
	{
		gKinhMach.Option(lpObj, 0);
	}
#endif
	//---------
#if (SACHTHUOCTINH_NEW == 1)
	gSachThuocTinh.Option(lpObj, 0);
#endif
	//---------
#if (PHUKIEN_NEW == 1)
	gPhuKien.Option(lpObj, 0);
#endif
	//---------
#if(BUFFSETDAME == 1)
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
...
#if(LEOTHAP == 1)
	lpObj->TowerCount = lpMsg->TowerCount;
	lpObj->TowerTime = lpMsg->TowerTime;

	if (lpObj->TowerCount < 0)
	{
		lpObj->TowerCount = 0;
	}
	if (lpObj->TowerTime < 0)
	{
		lpObj->TowerTime = 0;
	}
#endif
	...
	lpObj->JMoney = lpMsg->JMoney;
	lpObj->JBless = lpMsg->JBless;
	lpObj->JSoul = lpMsg->JSoul;
	lpObj->JChaos = lpMsg->JChaos;
	lpObj->JCreation = lpMsg->JCreation;
	lpObj->JLife = lpMsg->JLife;
	lpObj->JHarmony = lpMsg->JHarmony;
	lpObj->JGuaStone = lpMsg->JGuaStone;
	lpObj->JGemStone = lpMsg->JGemStone;
	lpObj->JBigStone = lpMsg->JBigStone;
	lpObj->JSmallStone = lpMsg->JSmallStone;
	...
	if (lpObj->JGuaStone < 0)
	{
		lpObj->JGuaStone = 0;
	}
	if (lpObj->JGemStone < 0)
	{
		lpObj->JGemStone = 0;
	}
	if (lpObj->JBigStone < 0)
	{
		lpObj->JBigStone = 0;
	}
	if (lpObj->JSmallStone < 0)
	{
		lpObj->JSmallStone = 0;
	}
#endif
		...
		if(gObj[SummonIndex].Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
		{
			...
			if (g_Survivor.ActiveEvent > -1 && g_Survivor.State == SURVIVOR_STATE_BATTLE)
			{
				if (g_Survivor.IsInSurvivorArea(lpObj) && g_Survivor.IsInSurvivorArea(lpTarget))
				{
					...
					if (gServerInfo.SurvivorItemByKillCharter == 1)
					{
						int PKEvent_Index = 0;
						unsigned char ItemsIndex[5] = { gServerInfo.SurvivorItemIndex1, gServerInfo.SurvivorItemIndex2, gServerInfo.SurvivorItemIndex3, gServerInfo.SurvivorItemIndex4, gServerInfo.SurvivorItemIndex5 };
						PKEvent_Index = ItemsIndex[rand() % 5];
						GDCreateItemSend(lpObj->Index, lpObj->Map, (BYTE)lpObj->X, (BYTE)lpObj->Y, GET_ITEM(gServerInfo.SurvivorItemType, PKEvent_Index), 0, 0, skill, 0, 0, -1, 0, 0, 0, 0, 0, 0xFF, 0);
					}
					...
					LogAdd(LOG_GREEN, "[PKEVENT] [%s] killed [%s] | Kill Count: %d", lpObj->Name, lpTarget->Name, lpObj->SurvivorKillCount);
...
#if (THAPBOSS_NEW == 1)
			gThapBossManager.MonsterDieProc(lpTarget, &gObj[SummonIndex]);
#endif
