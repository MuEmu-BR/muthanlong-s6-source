...
void CReiDoMU::MainProc() // OK
{
	...
	{ 
		if (this->WinnerNumber >= 0)
		{
			...
			if(this->m_RemainTimeWinner == 28)
			{
				...
				if (lpGuildInfo == 0)
				{
					...
					LogAdd(LOG_EVENT,"[Chiem Thanh] Guild Winner: %s", lpGuildInfo->Name);
					...
					for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
					{
						if(gObjIsConnectedGP(n) != 0 && gObj[n].Type == OBJECT_USER)
						{
							...
							{
								LogAdd(LOG_EVENT,"[Chiếm Thành] Member Reward: %s", gObj[n].Name);
			...
			if(this->m_RemainTimeWinner == 16)
			{
				if (this->rank1 >= 0)
				{
					...
					//Reward
					LogAdd(LOG_EVENT,"[Chiếm Thành] 1° %s (Guild: %s)", lpObj->Name, lpObj->GuildName);
			...
			if(this->m_RemainTimeWinner == 14)
			{
				if (this->rank2 >= 0)
				{
					...
					//Reward
					LogAdd(LOG_EVENT,"[Chiếm Thành] 2° %s (Guild: %s)", lpObj->Name, lpObj->GuildName);
					GDSetCoinSend(lpObj->Index, this->CRewardValue1, this->CRewardValue2, this->CRewardValue3,"KingOfMu");
				}
			}
			if(this->m_RemainTimeWinner == 12)
			{
				if (this->rank3 >= 0)
				{
					...
					//Reward
					LogAdd(LOG_EVENT,"[Chiếm Thành] 3° %s (Guild: %s)", lpObj->Name, lpObj->GuildName);
					GDSetCoinSend(lpObj->Index, this->CRewardValue1, this->CRewardValue2, this->CRewardValue3,"KingOfMu");
				}
			}
			if(this->m_RemainTimeWinner == 10)
			{
				if (this->rank4 >= 0)
				{
					...
					//Reward
					LogAdd(LOG_EVENT,"[Chiếm Thành] 4° %s (Guild: %s)", lpObj->Name, lpObj->GuildName);
					GDSetCoinSend(lpObj->Index, this->CRewardValue1, this->CRewardValue2, this->CRewardValue3,"KingOfMu");
				}
			}
			if(this->m_RemainTimeWinner == 8)
			{
				if (this->rank5 >= 0)
				{
					...
					//Reward
					LogAdd(LOG_EVENT,"[Chiếm Thành] 5° %s (Guild: %s)", lpObj->Name, lpObj->GuildName);
					GDSetCoinSend(lpObj->Index, this->CRewardValue1, this->CRewardValue2, this->CRewardValue3,"KingOfMu");
				}
			}
			...
			if(this->m_RemainTimeWinner <= 0)
			{
				...
				LogAdd(LOG_EVENT,"[Chiếm Thành] Finish");
		...
		{
			LogAdd(LOG_EVENT,"[Chiếm Thành] Finish - No Winner");
...
void CReiDoMU::SetState_START() // OK
{
	...
	LogAdd(LOG_EVENT,"[Chiem Thanh] Start");
...
void CReiDoMU::CommandReiDoMU(LPOBJ lpObj,char* arg) // OK
{
	...
	this->m_ActiveThongBao == 1;
	
	LogAdd(LOG_EVENT,"[Chiem Thanh] Nhân vật %s đã chiếm thành",lpObj->Name);
}
...
void CReiDoMU::StartKing()
{
	...
	LogAdd(LOG_EVENT,"[Set Chiem Thanh Start] At %2d:%2d:00",hour,minute);
