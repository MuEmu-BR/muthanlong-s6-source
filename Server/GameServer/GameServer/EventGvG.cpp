...
void CGvGEvent::ProcState_EMPTY() // OK
{
	if (this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if ((this->AlarmMinSave = (((this->m_RemainTime % 60) == 0) ? ((this->m_RemainTime / 60) - 1) : (this->m_RemainTime / 60))) != this->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1100), (this->AlarmMinLeft + 1));
			//gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1114));
		}
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1102));
		this->NoticeSendToAll(1, gMessage.GetMessage(1103), this->m_StandTime);
...
void CGvGEvent::ProcState_STAND() // OK
{
	...
	{
		this->NoticeSendToAll(0, gMessage.GetMessage(1112));
		this->SetState(GVG_EVENT_STATE_EMPTY);
		return;
	}
	...
	if (this->m_RemainTime <= 0)
	{
		if (this->TotalPlayer >= gServerInfo.m_GvGEventMinUsers)
		{
			...
			this->NoticeSendToAll(0, gMessage.GetMessage(1104));
			this->SetState(GVG_EVENT_STATE_START);
		}
		else
		{
			this->NoticeSendToAll(0, gMessage.GetMessage(1105));
			this->SetState(GVG_EVENT_STATE_EMPTY);
		}
	}
}
...
bool CGvGEvent::Dialog(LPOBJ lpObj, LPOBJ lpNpc)
{
	...
	{
		GCChatTargetSend(lpObj, lpNpc->Index, gMessage.GetMessage(1101));
...
bool CGvGEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1106));
		return 0;
	}
	...
	if (lpObj->Guild == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1109));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1107));
		return 0;
	}

	if (lpObj->GuildStatus == G_MASTER && this->CheckReqItems(lpObj) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1108));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1110), this->MaxMasterReset);
		return 0;
	}

	if (lpObj->GuildStatus == G_MASTER && this->AddGuild(lpObj->GuildNumber) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1106), this->MaxMasterReset);
		return 0;
	}
...
bool CGvGEvent::AddGuild(int index) // OK
{
	...
	for (int n = 0; n < MAX_GVGEVENT_GUILD; n++)
	{
		...
		this->Guild[n].Gate = this->StartGate + n; // doan set gate
		return 1;
	}
	return 0;
}
...
void CGvGEvent::UserDieProc(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	this->NoticeSendToAll(0, gMessage.GetMessage(1111), lpGuild->Point, lpTarget->GuildName);
}
	...
	for (int n = 0; n < MAX_GVGEVENT_GUILD; n++)
	{
		if (this->Guild[n].Guild >= 0)
		{
			...
			if (this->Guild[n].Rank == 1)
			{
				...
				if (lpGuildInfo != NULL)
				{
					this->NoticeSendToAll(0, gMessage.GetMessage(1113), lpGuildInfo->Name);
				}
