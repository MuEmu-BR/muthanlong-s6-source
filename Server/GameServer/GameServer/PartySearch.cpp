...
void CPartySearch::PartySearchAddToList(PMSG_RECV_PARTYSEARCH_ADD* lpMsg, int aIndex)
{
	...
	if (lpMsg->SystemActive == true)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn đã tham gia Party!");
			LogAdd(LOG_RED, "[check] party player send");
			return;
		}
		...
		if (it == this->m_Registered.end())
		{
			...
			if (info.RequirePassword)
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[%s] đã tạo nhóm + mật khẩu [%s ,%d %d]", gObj[aIndex].Name, gMapManager.GetMapName(gObj[aIndex].Map), gObj[aIndex].X, gObj[aIndex].Y);
			else
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[%s] đã tạo nhóm [%s ,%d %d]", gObj[aIndex].Name, gMapManager.GetMapName(gObj[aIndex].Map), gObj[aIndex].X, gObj[aIndex].Y);
		}
		else
		{
			it->second = info;
...
void CPartySearch::RequestParty(PMSG_PARTY_REQ_REQ* lpMsg, int aIndex)
{
	...
	if (info->second.RequirePassword)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Đã xảy ra lỗi.Liên hệ với quản trị viên.");
			return;
		}

		if (strcmp(info->second.Password, lpMsg->Password) != 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn đã nhập sai mật khẩu. Thử lại...");
			return;
		}
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn không thể thêm mình vào Party!");
		return;
	}
