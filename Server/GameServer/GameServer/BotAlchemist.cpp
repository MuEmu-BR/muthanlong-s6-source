		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 4)
				{
					...
					this->bot[BotNum].VipMoney = lpMemScript->GetAsNumber();
...
BYTE ObjBotAlchemist::Alchemy(int aIndex, int BotNum)
{
	...
	if (this->bot[BotNum].PCPoint > 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn cần %d WcoinP để cộng hưởng", this->bot[BotNum].PCPoint);
			goto Cancel;
		}
	}
	...
	if (this->bot[BotNum].VipMoney > 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn cần %d WcoinC để cộng hưởng", this->bot[BotNum].VipMoney);
	...
	if (fitem == -1 || sitem == -1)
	{
		ChatSend(&gObj[this->bot[BotNum].index], "Bạn phải đặt hai mục xa nhau!");
	...
	if (this->bot[BotNum].OnlySameType == 1)
	{
		...
		if (fType != sType)
		{
			ChatSend(&gObj[this->bot[BotNum].index], "Bạn phải đặt hai mặt hàng cùng loại!");
	...
	if (this->bot[BotNum].OnlyLowerIndex == 1)
	{
		...
		if (fLevel > sLevel)
		{
			if (fLevel > (sLevel + 10))
			{
				ChatSend(&gObj[this->bot[BotNum].index], "Item cap cao hon khong duoc cong huong tu item cap thap hon!");
	...
	{
		ChatSend(&gObj[this->bot[BotNum].index], "Item than thanh toi khong tiep nhan no, hay dat item khac vao!");
	...
	else if (this->bot[BotNum].PCPoint > 0)
	{
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, this->bot[BotNum].PCPoint, 0,0, 0); //
		gCashShop.CGCashShopPointRecv(lpObj->Index);
	}
...
BOOL ObjBotAlchemist::TradeOpen(int index, int nindex)
{
	...
	{
		ChatSend(&gObj[this->bot[number].index], "Bạn hãy là Vip rồi quay lại gặp tôi!");
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn cần %d WCoinC để cộng hưởng!", this->bot[number].VipMoney);
		LogAdd(LOG_RED,"[BotAlchemist][Bot:%d](%s) Insuficent WCoinC",number,gObj[index].Account);
		return 1;
	}

	if ((gObj[index].Coin2 - this->bot[number].PCPoint) < 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Bạn cần %d WCoinP để cộng hưởng!", this->bot[number].PCPoint);
		return 1;
	}
	if (this->Enabled == TRUE)
	{
		...
		else
		{
			...
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Tôi cần %d WCoinC để cộng hưởng", this->bot[number].VipMoney);
			}

			if (this->bot[number].PCPoint > 0)
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Tôi cần %d WCoinP để cộng hưởng",this->bot[number].PCPoint);

			}

			if (this->bot[number].Zen > 0)
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Tôi cần %d Zen!", this->bot[number].Zen);
			}
		}
	}
	return 1;
