...
public:
...
#if(CUSTOM_MIXDEV == 1)
	void ReloadCustomMix();//mc bot
#endif

#if(VANTHANH > 4)
	void ReloadVanThanhInfoCpanel();//mc bot
#endif

	void ReloadVanThanhInfo();//mc bot
	void ReloadVanThanhInfo(char* section, char* path);
	...
	short SurvivorTimeData;
	short SurvivorTimeThongBaoDangKy;
	short SurvivorTimeChuanBiBatDau;
	...
	int   SurvivorItemByKillCharter;
	int   SurvivorItemType;
	int   SurvivorItemIndex1;
	int   SurvivorItemIndex2;
	int   SurvivorItemIndex3;
	int   SurvivorItemIndex4;
	int   SurvivorItemIndex5;

	int   SurvivorWinWCoinC;
	int   SurvivorWinWCoinP;
	int   SurvivorWinThanhTuu;
...
public:
	...
	int CustomAttackDiChuyen;

	//-- Skill
	int SkillDocDW;
	int ThoiGianDameDocDW;

	int ThoiGianDameDocNguHanh;
	int ElfNoBoltArrow;
	...
	int m_VuKhiRongMixRate[MAX_ACCOUNT_LEVEL];

	//27-03-2022
	int	m_CapNhapXepHangClassAll;
	int	m_CapNhapXepHangClass;
	int	m_CapNhapXepHangTopNap;
	int	m_CapNhapXepHangEvent;

	//25-03-2022
	int	m_ThongBaoReset;

	//23-03-2022
    int	m_ThongBaoOnline;

	int FenrirofHorse_Drop;
	int FenrirofHorse_Min;
	int FenrirofHorse_Max;

	//--
#if(LEOTHAP == 1)
	int m_LeoThapEvent;
	int m_LeoThapMaxUser;
	int m_LeoThapMap;
#endif

#if(THAPBOSS_NEW == 1)
	int m_ThapBossSwitch;
#endif

#if(BOSS_NHEN_TIME == 1)
	int m_BossNhenSwitch;
#endif
