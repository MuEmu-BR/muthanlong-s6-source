...
#if(DKTK1 == 1)
		case 0x31:
			GJRegistroMainRecv((SDHP_REGISTRO_MAIN_RECV*)lpMsg, index);
			break;
#endif
#if(DKTK2 == 1)
		case 0x31:
...
#if(DKTK1 == 1)
void GJRegistroMainRecv(SDHP_REGISTRO_MAIN_RECV* lpMsg, int index) // OK
{
	
	if (gQueryManager.ExecQuery("INSERT INTO dbo.MEMB_INFO(memb___id, memb__pwd, memb_name, mail_addr, fpas_ques, fpas_answ, sno__numb, post_code, bloc_code, ctl1_code) VALUES ('%s', '%s', 'MuThanLong','%s@gmail.com', 'question', 'answer', '%s', 1, 0, 0)", lpMsg->account, lpMsg->password, lpMsg->account, lpMsg->numcode) == TRUE)
	{
		gQueryManager.Close();
...
void GJRegistroMainRecv(SDHP_REGISTRO_MAIN_RECV* lpMsg, int index) // OK
{
	...
	//gQueryManager.GetAsString("memb___id", user, sizeof(user));

	//if (strcmp(lpMsg->account, user) != 0)
	//{
	//	return;
	//}
	...
	//if (strcmp(lpMsg->account, account) != 0 && strcmp(lpMsg->account, account) != 0)
	//{
	//	gQueryManager.Close();
	//	return;
	//}
...
/*	char account[11] = { 0 };
	if (gQueryManager.ExecQuery("SELECT count(*) FROM MEMB_INFO WHERE LOWER(memb___id)", lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		if (strcmp(lpMsg->account, account) != 0 && strcmp(lpMsg->account, account) != 0)
		{
			gQueryManager.Close();
	...
	else*/ if (gQueryManager.ExecQuery("INSERT INTO dbo.MEMB_INFO(memb___id, memb__pwd, memb_name, mail_addr, fpas_ques, fpas_answ, sno__numb, post_code, bloc_code, ctl1_code) VALUES ('%s', '%s', 'MuThanLong','%s@gmail.com', 'question', 'answer', '%s', 1, 0, 0)", lpMsg->account, lpMsg->password, lpMsg->account, lpMsg->numcode) == TRUE)
	{
		gQueryManager.Close();
