...
//**********************************************//
#if(DKTK1 == 1)
struct PMSG_DANGKY_SEND
{
	PBMSG_HEAD h; // C1:B1:00
};

struct PMSG_REGISTER_ACCOUNT_RECV
{
	PSBMSG_HEAD h; // C3:F1:01
	char account[10];
	char password[10];
	//char name[50];
	//char phone[30];
...
void GCMuunPetTime(MUUNPETTIME_SENDCLIENT* pMsg);
...
void GCMuunPetAFKTime(MUUNPETAFK_SENDCLIENT* pMsg);
#endif
...
void GCLEOTHAPShowWindow(PMSG_LEOTHAP_REQ* pMsg);
#endif
