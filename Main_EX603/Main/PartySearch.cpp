...
void cPartySearch::draw_party_search()
{
	...
	//gCItemSetOption.ItemTooltipS15(StartX, StartY - 305, 223, 330, 0.0, 0);
	gCentral.PrintDropBox(StartX, StartY - 305, 225, 330, 0, 0);
	...
	// ----
	gInterface.DrawText(eGold, StartX + 10, 60, 210, 3, "Tìm Nhóm");
	// ----
	...
	// ----
	gInterface.DrawGUI(OBJECT_PARTYSETTINGS_DIV, StartX, gInterface.Data[OBJECT_PARTYSEARCH_MAIN].Y + 40); // Äåëèòåëü áîëüøîé
	// ----
	...
	// ----
	gInterface.DrawGUI(OBJECT_PARTYSETTINGS_DIV, StartX, gInterface.Data[OBJECT_PARTYSEARCH_MAIN].Y + 60); // Äåëèòåëü áîëüøîé
	// ----
	...
	for (int i = 0; i < gPartySearch.ListsCount; i++)
	{
		...
		gInterface.DrawText(eWhite, StartX + 140, gInterface.Data[OBJECT_PARTYSEARCH_MAIN].Y + 86 + (20 * i), 50, 3, "%d / 10", info->Count);
...
PARTYLIST* cPartySearch::GetPartyList(int index)
{
	...
	if (gPartyList[index].Count >= MAX_USER_IN_PARTY)
	{
