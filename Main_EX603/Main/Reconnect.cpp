...
void ReconnectMainProc() // OK
{
	...
	if (AccountSetted == 0)
	{
		strcpy((char*)0x0E8C260, ReconnectAccount);
		AccountSetted = 1;
	}
...
void RecvDC(DISCONNECT_REQ * aRecv)
{
	////console.Log(__FUNCTION__, "Recv");
	ReconnectSetInfo(RECONNECT_STATUS_DISCONNECT, RECONNECT_PROGRESS_NONE, 0, 0);
	((void(__thiscall*)(void*))0x0063A180)((void*)0x08793750);
}
...
void ReconnecGameServerLoad() // OK
{
	if (ReconnectCreateConnection(GameServerAddress, GameServerPort) != 0)
	{
		*(DWORD*)(MAIN_CONNECTION_STATUS) = 1;
		...
		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_CONNECTED, gProtect.m_MainInfo.ReconnectConnectCharacterTime, 30000);
	}
}
...
void ReconnectOnCloseSocket() // OK
{
	...
	{
		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_NONE, gProtect.m_MainInfo.ReconnectConnectAccountTime, gProtect.m_MainInfo.ReconnectTime);
		...
		memcpy(ReconnectName, gObjUser.lpPlayer->Name, sizeof(ReconnectName));
	}
}
