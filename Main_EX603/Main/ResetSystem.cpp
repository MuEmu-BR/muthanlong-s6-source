	...
	if (this->m_ResetData.UseItemReward)
	{
		for (int i = 0; i < this->m_ResetData.ItemReward.ItemCount; i++)
		{
			if (gInterface.IsWorkZone2(start_x + main_width / 2, 225 + (i * 8), start_x + main_width - 30, 232 + (i * 8)))
			{
				...
				gNewInterface.DrawBarForm(X, Y, barWidth, barHeight, 0.04, 0.34, 0.0, 1.0);//ReqItems
				...
				gInterface.DrawItem(X, Y, barWidth, barHeight, item->ID, SET_ITEMOPT_LEVEL(item->MinLevel), ExcOpt, item->IsAncient, 0);
			}
		}
	}
	// ----
...
void ResetSystem::req_reset_finish()
{
	...
	pMsg.h.set(0xFF, 0x01, sizeof(pMsg));
