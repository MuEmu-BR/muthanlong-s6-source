...
void InitEncDec()
{
	SetCompleteHook(0xE9, 0x004FC7AC, &EncDecOpenJpegBuffer1);
...
__declspec(naked) void EncDecOpenJpegBuffer1()
{
	...
	_asm
	{
		Push 2
			...
			Call[Address4]
			Add Esp, 0x0C
			Push EncDecJpegBufferSize
			Call[Address6]
			Add Esp, 0x04
			Push 0x18
			Push 0
			Mov EncDecJpegBuffer, Eax
			Push EncDecJpegBufferSize
			...
			Test Eax, Eax
			Je EXIT
			Mov Edx, EncDecJpegBufferSize
			Sub Edx, 0x18
			Push Edx
			Mov Eax, EncDecJpegBuffer
			Add Eax, 0x18
			Push Eax
