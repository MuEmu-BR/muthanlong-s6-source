...
void ExWinQuestSystem::Draw()
{
	...
	gInterface.DrawFormat(eGold, (int)StartX + 10, (int)StartY + 10, 210, 3, "Hệ Thống Nhiệm Vụ");
	...
	gInterface.DrawText(eAncient, ContentX + 30, MissionY + 2, 170, 3, "Nhiệm Vụ");
	...
	for (int m = 0; m < EW_MAX_MISSION; m++)
	{
			...
			this->ewdata[a].MissionType[m] == Searching)
		{
			...
			{
				gInterface.DrawText(bColor, MissionX + 30 + 5 + 15, CountMissionY, 200, 1, "- %s: %d/%d", GetItemName(GET_ITEM(this->ewdata[a].MissionID[m], this->ewdata[a].MissionIndex[m])), ewdata[a].Count[m], ewdata[a].MissionCount[m]);
			}
			else if (this->ewdata[a].MissionType[m] == Searching)
			{
				gInterface.DrawText(bColor, MissionX + 30 + 5 + 15, CountMissionY, 200, 1, "- %s", GetItemName(GET_ITEM(this->ewdata[a].MissionID[m], this->ewdata[a].MissionIndex[m])));
			}
		}
	}
	// ------------------------------------
	...
	gInterface.DrawText(eAncient, ContentX + 30, RewardY + 12, 170, 3, "Phần Thưởng");
	...
	for (int r = 0; r < EW_MAX_REWARD; r++)
	{
			...
			this->ewdata[a].RewardType[r] == ewBuff)
		{
			...
			else if (this->ewdata[a].RewardType[r] == ewItem)
			{
				...
				{
					gInterface.DrawText(eGold, StartX + 40, CountRewardY + 5, 150, 3, "%s +%d x %d",
				...
				{
					gInterface.DrawText(eGold, StartX + 40, CountRewardY + 5, 150, 3, "%s x %d",
	...
	if (this->ewdata[a].Start)
	{
		gInterface.DrawFormat(eWhite, StartX + (MainWidth / 2) - (70 / 2), StartY + MainHeight - 25, 59, 3, "Làm Xong");
	}
	else
	{
		gInterface.DrawFormat(eWhite, StartX + (MainWidth / 2) - (90 / 2), StartY + MainHeight - 25, 59, 3, "Chấp Nhận");
	}
	//------
	if (!Finish)
	{
...
#if(VANTHANH > 3)
		if (gObjUser.m_MapNumber == 83
			|| gObjUser.m_MapNumber == 84
			|| gObjUser.m_MapNumber == 85
			|| gObjUser.m_MapNumber == 86
			|| gObjUser.m_MapNumber == 87
			|| gObjUser.m_MapNumber == 88
			|| gObjUser.m_MapNumber == 89
			|| gObjUser.m_MapNumber == 90
			)
		{
			return;
		}
...
void ExWinQuestSystem::DrawOpenWQ()
{
	...
	if (gInterface.Data[eSACHTHUOCTINH_INTERFACE].OnShow)//
	{
		return;
	}
	...
	if (gInterface.IsWorkZone(OPEN_QUEST)) // nếu chuột trong khu vực bảng shop thì ko click di chuyển nv
	{
		pSetCursorFocus = true;
	}
	...
	if (gInterface.IsWorkZone(OPEN_QUEST))
	{
		...
		gInterface.DrawToolTip(560, 210, "Nhiệm Vụ");
		// ----
...
//// ----------------------------------------------------------------------------------------------
//
void ExWinQuestSystem::CG_AcneptQuest()
{
